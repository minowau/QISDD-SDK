describe('@qisdd/fintech utilities', () => {
  const maskAccount = (acct) => acct.replace(/.(?=.{4})/g, '*');
  const calcFee = (amount, pct = 0.025) => Math.round(amount * pct * 100) / 100;
  ...
  test('mock payment processor flow', async () => {
    class MockProcessor { async charge(amount) { return { success: true, amount }; } }
